...
class MondogrossoProcessOrdersTests extends Specification {
	...
	"samples" should {
		...
		//まだ良くわかってない。近づいてきた気がする。
//		"+>A(else:over:vie else:over:vie)<(S T)	<に複数の要素があるケース" in {
...
//		}
		...
		//OK
		">A(else:over:vie else:over:vie)<S+>AB(elseB:overB:vieB elseB:overB:vieB)<SB 複数のProcessフルセット	" in {	
...
//		//OK
//		"+>A(else:over:vie else:over:vie)	<のあとにIdentityが一つあるケース" in {
//			
//			val input = "+>A(else:over:vie else:over:vie)<S"
//			val json = "{\"A\":{\"type\":\"process\",\"class\":\"A\",\"exec\":\"exec\",\"kv\":{\"key1\":\"value1\",\"key2\":\"value2\"}},\"Z\": {\"type\": \"process\",\"class\": \"Z\",\"exec\": \"exec\",\"kv\": {\"key1\": \"value1\",\"key2\": \"value2\"}}}}}"
//			val p = new MondogrossoProcessParser(input, json)
//		}
		...
		//OK
//		"+>A(else:over:vie else:over:vie)	<が無いケース" in {
//			
...
//		}
		...
		/*
		 * +>A>B(else:over:vie else:over:vie)>C>D>E<H>F>G
